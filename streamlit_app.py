...
period_selection = st.sidebar.selectbox(
    ...
    key="period_selector"  # Explicit key for proper widget state management
)
...
selected_date = st.sidebar.date_input(
    ...
    key="daily_date_selector"  # Explicit key for proper widget re-rendering
)
...
# Check what changed - DATE CHANGES ALWAYS TAKE PRIORITY
...
if date_changed:
    # DATE CHANGE ALWAYS SWITCHES TO DAILY MODE
    display_mode = "daily"
    st.session_state.last_selected_date = selected_date
    st.session_state.last_interaction = "daily"
elif period_changed and period_selection != "None":
    # Period change only if date didn't change
    display_mode = "period"
    ...
    # Explicit "None" selection means daily
    display_mode = "daily"
    st.session_state.last_interaction = "daily"
else:
    # Nothing changed - use last interaction mode
    display_mode = st.session_state.get('last_interaction', 'daily')

# Update period tracking for next iteration
st.session_state.last_period_selection = period_selection
...
if "Demo" in data_source:
    ...
    # These will be overridden by Supabase data
    max_temp = 110
    peak_price = 150
    water_cost = 3.24

# Run optimization button - ONLY for daily mode (historical periods are preloaded)
        ...
        with st.spinner("Optimizing data center operations..."):
            ...
            if use_supabase:
                ...
                try:
                    ...
                    if results:
                        st.success("✅ Optimization complete using Supabase data!")
...
# Display results based on display mode (daily from calendar or period from dropdown)
            ...
            try:
                ...
                if period_selection in days_map:
                    ...
                    if period_summary and period_summary.get('total_savings', 0) > 0:
                        ...
                        with col1:
                            # Update label based on projection status
                            if period_summary.get('is_projection', False):
                        ...
                        if days >= 30:
                            # Convert days to months for the method call
                        ...
                        # Don't reassign period_selection - let the user control it via the selectbox
            except Exception as e:
    ...
    if data_interface.supabase:
        ...
        if not history_df.empty:
            ...
            if all(col in history_df.columns for col in display_cols):
                ...
                st.info("No optimization history available yet. Run some optimizations to see historical data!")
        else:
            st.info("No optimization history available yet. Run some optimizations to see historical data!")
    else:
